...
const loginController = async (req, res) => {
  try {
    ...
    return res
      .status(404)
      .json({ status: "BAD", message: "Something went wrong" });
  }
};
  ...
  try {
    ...
    if (user) {
      ...
      if (userWithUsername) {
        ...
        const userWithContactNumber = await userModel.findOne({
