...
const Book = () => {
  ...
  return (
    <>
      ...
      {!userLoading ? (
        <>
          <div className="w-full h-full xl:flex xl:flex-row sm:flex-col overflow-y-auto">
            <div className="Mistri xl:w-[40%] xl:h-full sm:w-full sm:h-[120%] bg-white">
              {mistris ? (
                mistris
                  .slice()
                  .reverse()
                  .map((mistri, index) => (
                    <div
                      key={index}
                      className="w-full xl:h-full  overflow-y-auto noScroll">
                      <div className="Image xl:w-full sm:w-full xl:h-[43%] sm:h-96 overflow-hidden ">
                        <img
                          src={mistri.profileImage}
                          className="w-full xl:h-full sm:h-full object-cover hover:scale-110 transition-all "
                        />
                      </div>
                      <div className="Charges flex items-center justify-between">
                        <span className="px-3 py-1 bg-indexColor text-white rounded-full m-1">₹{mistri.charges}/</span>
                        <span>
                          <svg
                      ...
                      <div className="Address w-full flex items-center justify-center p-1">
                        <div className="w-[80%] flex items-center justify-center">
                          <p className="overflow-hidden truncate">{mistri.address}</p>
                        </div>
                      </div>
                      <div className="Name w-full flex flex-col items-center justify-center p-1">
                        <div className="xl:w-[60%] sm:w-[50%] flex items-center justify-center">
                          <h3 className="text-2xl font-bold overflow-hidden truncate">{mistri.mistriname}</h3>
                        </div>

                        <p>{mistri.profession}</p>
                      </div>
                      <div className="Inputs-Charges-Date-Time w-full flex flex-col   ">
                        <p className=" text-xl font-bold">Mistri charges</p>
                        <input
                          name="charges"
                          onChange={(e) => setCharges(e.target.value)}
                          type="text"
                          className="w-full h-[60px]  border-2 outline-none border-zinc-300 border-x-white px-2 mt-1 mb-1"
                          defaultValue={mistri.charges}
                          placeholder="Your price"
                        />
                        <p className=" text-xl font-bold">Select date for appointment</p>
                        <input
                          name="orderDate"
                          onChange={(e) => setDate(e.target.value)}
                          type="date"
                          className="w-full h-[60px]  border-2 cursor-pointer outline-none border-zinc-300 border-x-white px-2 mt-1 mb-1"
                          placeholder="Time"
                        />
                        <p className=" text-xl font-bold">Select time for appointment</p>
                        <input
                      ...
                      <div className="Confirm-Booked-Button w-full flex flex-col items-center justify-center xl:relative sm:fixed sm:bottom-[70px] mt-2 xl:bottom-1 z-50">
                        {user ? (
                          // Check if there are any orders with the current mistri._id
            ...
            <div className="User xl:w-[60%] xl:h-full sm:w-full sm:mb-16 xl:mb-0 p-2 sm:pb-20">
              {/* <h3 className="text-4xl font-bold">Your details.</h3> */}
              {userFeilds.map((userFeild, index2) => (
                <label
                  key={index2}
                  className="flex mt-2 items-center gap-2 w-full h-[40px] px-2 bg-white  rounded-md overflow-hidden">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    aria-label="Settings"
                    className="p-1"
              ...
              <div className="w-full flex items-center justify-center">
                <Button className="bg-gray-300  mt-4 px-2 py-3  w-[90%] rounded-md text-white flex items-center justify-center">
                  Edit profile
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    aria-label="Settings"
